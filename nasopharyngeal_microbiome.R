...
########################
diversity_data<-data.frame("total_reads" =  phyloseq::sample_sums(phylo_seq),
                           "Chao1" = phyloseq::estimate_richness(phylo_seq, measures = "Chao1"),
                           "Observed" = phyloseq::estimate_richness(phylo_seq, measures = "Observed"),
                           "Simpson" = phyloseq::estimate_richness(phylo_seq, measures = "Simpson"),
                           "Shannon" = phyloseq::estimate_richness(phylo_seq, measures = "Shannon"),
                           "symptoms"= phylo_seq@sam_data[["Symptoms"]])

cor(diversity_data$total_reads,diversity_data$Simpson)
cor(diversity_data$total_reads,diversity_data$Shannon)

model1 <- lm(total_reads ~ Shannon, data = diversity_data)
summary(model1)
confint(model1,param="Shannon")

#predict(model1, interval = 'confidence')

model2 <- lm(total_reads ~ Simpson, data = diversity_data)
summary(model2)
confint(model2,param="Simpson")
...
#ADONIS test
a <- vegan::adonis2(clr_dist_matrix ~ phyloseq::sample_data(physeq)$Samples)
print(a)

dispr <- vegan::betadisper(clr_dist_matrix, phyloseq::sample_data(physeq)$Samples)
dispr

dispr1 <- vegan::betadisper(clr_dist_matrix, phyloseq::sample_data(physeq)$Symptoms)
dispr1
...
D1 <- ggviolin(df, x = "Samples", y = "DysbiosisIndex",add = "boxplot", fill = "Samples", palette = c( "#a6cee3","#b2df8a"))+
  stat_compare_means(comparisons = sampletype.pairs) + geom_jitter(shape=16,position = position_jitter(0.07)) 
print(D1)

##FIGURE 2H:

D_1 <- ggviolin(df, x = "Symptoms", y = "DysbiosisIndex",add = "boxplot", fill = "Symptoms", palette = c("#a6cee3", "#b2df8a", "#fdbf6f"))+
  stat_compare_means(comparisons = symptoms.pairs) + geom_jitter(shape=16,position = position_jitter(0.07)) 
print(D_1)


wilcox.test(df$DysbiosisIndex ~ df$Samples, data = df, conf.int = TRUE)  
...
adonis(UN.dist1 ~ Samples+Symptoms, data = samplesdf)
...
#Run PERMANOVA on distances.
adonis(UN.dist ~ Samples+Symptoms, data = samplesdf)
...
#######################################   family MCLust and correlation heatmap ###############
...
clr_dist<-as.matrix(clr_dist_matrix)
#ADONIS test
a <- vegan::adonis(clr_dist_matrix ~ phyloseq::sample_data(ps_genus)$Samples)
print(a)

dispr <- vegan::betadisper(clr_dist_matrix, phyloseq::sample_data(ps_genus)$Samples)
dispr

dispr1 <- vegan::betadisper(clr_dist_matrix, phyloseq::sample_data(ps_genus)$Symptoms)
dispr1

plot(dispr, main = "Ordination Centroids and Dispersion Labeled ",sub = "",label = "TRUE",label.cex =0.6)
plot(dispr1, main = "Ordination Centroids and Dispersion Labeled ", sub = "",label = "TRUE",label.cex =0.5,face="bold")

ord.meta <- data.frame(sample_data(ps_genus))
ord.asvs <- otu_table(ps_genus)
bc.pcoa <- ordinate(ord.asvs, method = 'PCoA',distance = "bray")


# Calculate distance matrices
set.seed(1)
bc.envfit.pcoa <- envfit(bc.pcoa$vectors[,1:2] ~ Samples,
                         data = ord.meta,
                         display = 'sites',
                         na.rm = T)
bc.envfit.pcoa

bc.envfit.pcoa1 <- envfit(bc.pcoa$vectors[,1:2] ~ Symptoms,
                          data = ord.meta,
                          display = 'sites',
                          na.rm = T)
bc.envfit.pcoa1

pcoa.df<-data.frame(bc.pcoa$vectors[,1:2])
centroid<-bc.envfit.pcoa$factors$centroids[1,]

dist<-data.frame(apply(pcoa.df,1,function(x,centroid){(sqrt((x[1]-centroid[1])^2+(x[2]-centroid[2])^2))},centroid))

d1<-ord.meta[,4:5]
df_genus<-cbind(dist,d1)
colnames(df_genus)<-c("DysbiosisIndex","Samples","Symptoms")
head(df_genus)

# create a list of pairwise comaprisons
Sampletype <- factor((df_genus$Samples),level=c("Infected","Control")) # get the variables
Sampletype<-levels(Sampletype)

symptoms <- factor((df_genus$Symptoms),level=c("Asymptomatic","Control","Symptomatic"))
symptoms<-levels(symptoms)
...
D1 <- ggviolin(df_genus, x = "Samples", y = "DysbiosisIndex",add = "boxplot", fill = "Samples", palette = c( "#a6cee3","#b2df8a"))+
  stat_compare_means(comparisons = sampletype.pairs) + geom_jitter(shape=16,position = position_jitter(0.07)) 
print(D1)


D1 <- ggviolin(df_genus, x = "Symptoms", y = "DysbiosisIndex",add = "boxplot", fill = "Symptoms", palette = c("#a6cee3", "#b2df8a", "#fdbf6f"))+
  stat_compare_means(comparisons = symptoms.pairs) + geom_jitter(shape=16,position = position_jitter(0.07)) 
print(D1)


wilcox.test(df_genus$DysbiosisIndex ~ df_genus$Samples, data = df_genus, conf.int = TRUE)  
kruskal.test(df_genus$DysbiosisIndex ~ df_genus$Symptoms, data = df_genus)
...
siggenusotus <- LDA_otus %>% select(-OTUs) 
row.names(LDA_taxa) <- LDA_taxa $OTUs
siggenustaxa <- LDA_taxa %>% select(-OTUs) 
row.names(samples_df) <- samples_df$Sample
samplesdf <- samples_df %>% select(-Sample) 
...
#################################### 12 _non_significant genera ROC_AUC #####################################
...
##############################
################# species_pairwise boxplot ###########
species_CA <- read.csv("D:/Project/nasopharyngeal_microbiome_analysis/specieslevel/species_asymptomatic_control.csv")
species_CS <- read.csv("D:/Project/nasopharyngeal_microbiome_analysis/specieslevel/species_control_symptomatic.csv")
species_AS <- read.csv("D:/Project/nasopharyngeal_microbiome_analysis/specieslevel/species_symptomatic_asymptomatic.csv")
...
require(ggplot2)
library(ggsignif)
ggplot(data = species_CA, aes(x=Species, y=Abundance)) + 
  geom_boxplot(aes(fill=Symptoms))+ylab("Relative Abundance")+
  scale_fill_manual(values = c("#8828B8","#2886B8")) +
  theme(axis.text=element_text(size=10,face="bold"))+
  stat_compare_means(method = "wilcox.test",aes(group = Symptoms), label = "p.signif")+
  coord_flip()


ggplot(data = species_CS, aes(x=Species, y=Abundance)) + 
  geom_boxplot(aes(fill=Symptoms))+ylab("Relative Abundance")+
  scale_fill_manual(values = c("#2886B8","#B8AF28")) +
  theme(axis.text=element_text(size=10,face="bold"))+
  stat_compare_means(method = "wilcox.test",aes(group = Symptoms),label = "p.signif")+
  coord_flip()


ggplot(data = species_AS, aes(x=Species, y=Abundance)) + 
  geom_boxplot(aes(fill=Symptoms))+ylab("Relative Abundance")+
  scale_fill_manual(values = c("#8828B8","#B8AF28")) +
  theme(axis.text=element_text(size=10,face="bold"))+
  stat_compare_means(method = "wilcox.test",aes(group = Symptoms), label = "p.signif",)+ 
  coord_flip()
...
df_comb1 = expand.grid(names(df), names(df),  stringsAsFactors = F) %>% set_names("X1", "X2")
df_comb1
# apply function to each variable combination
df_res1=map2_df(df_comb1$X1, df_comb1$X2, f)
df_res1
